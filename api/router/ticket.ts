...
router.post("/add", async (req: Request<Ticket>, res: Response<Ticket>) => {
    ...
    await con.query(`UPDATE users SET having_money = ? WHERE user_id = "${req.body.user_id}"`, [setMoney]);
    const [[ticket]] = await con.query<RowDataPacket[]>(
        "SELECT * FROM ticket WHERE ticket_id = ?",
...
router.post("/add/formation", async (req: Request<FormationTicket>, res: Response<FormationTicket>) => {
    if (req.body.type !== TicketType.EXACTA && req.body.type !== TicketType.TRIFECTA) throw new Error("cannot buy this type of ticket")
    const [checkMoneyRows] = await con.query<RowDataPacket[]>("SELECT having_money FROM users WHERE user_id = ?", [req.body.user_id])
    if (checkMoneyRows.length === 0) throw new Error("The User is not found.")
    const ticket = KRAFormation(req.body.type, req.body.bet, req.body.f, req.body.s, req.body.t, req.body.user_id)
    const setMoney = checkMoneyRows[0].having_money - parseInt(req.body.bet) * ticket.length
    if (setMoney < 0) throw new Error("You do not have enough money to complete the bet.")
    ticket.map(async t => {
        await con.query("INSERT INTO ticket (user_id, horse, type, option, optNum, bet, race) VALUES (?, JSON_ARRAY(?), ?, ?, ?, ?, ?)", [
            req.body.user_id,
            t.horse,
            t.type,
            req.body.option,
